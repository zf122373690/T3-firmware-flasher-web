...
浏览器出于安全设计，首次使用 Web Serial 必须手动授权一次。如果需要**零交互**的产线式体验，请使用随项目附带的 Electron 桌面版：
...
桌面版特性：
- **免授权** - 主进程自动放行串口权限，不弹出设备选择框，串口自动选择
- **插线即烧** - 插入设备自动识别、自动连接、自动开始升级
- **网页同源** - 复用同一套页面代码，浏览器版功能完全一致

> 浏览器版仍保留：首次授权一次后，之后每次插入设备同样自动识别、自动烧录。
